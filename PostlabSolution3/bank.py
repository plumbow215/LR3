...
class Bank:
    ...
    def __str__(self):
        ...
        Sorted_Accounts = sorted(self.accounts.values(), key=lambda acc: (acc.getName().lower(), int(acc.getPin())))
...
def main(number = 10, fileName = None):
    ...
    number."""
    newBank = createBank(5)

    for account in newBank.accounts.values():
        print(account)
    ## testAccount()
##    if fileName:
##        bank = Bank(fileName)
